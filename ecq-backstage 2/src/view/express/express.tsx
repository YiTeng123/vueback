...
name:'express',
    setup(prop, ctx) {
        ...
        const getResult = async () => {
            ...
            let { data } = res!
            data = data.filter(item => { return item.looktestPoint[0] !== "表情评测无数据" })
            result.push(...data)
        }
            ...
            <>
                <div class="express">
                    ...
                    >
                        ...
                        <el-table-column prop="expresstionMp4" label="视频路径" align='center'  >
                            {{
                                default: ({row:{expresstionMp4}}: {row:backRes }) => (
                                    <div onClick={()=>{console.log('232')}}>{expresstionMp4}</div>
                                )
                            }}
                        </el-table-column>
