...
import {HookService} from '@/Hooks/useClass.hooks'
export default defineComponent({
    async setup(prop, ctx) {
        ...
        const getResult = async () => {
            const [err, res] = await to<ResultData<ReArray>>(http.get<ReArray>('/userApi/getpoints'))
            const { data } = res!
            result.push(...data)
        }
        // onMounted( async() => {
        ...
        return () => (
            <div class="general">
                ...
                {result.length ? <><el-button onClick={() => handleButton('你确定要导出数据吗')}>导出1</el-button>  <el-button onClick={HookService.debuounce1(()=>{console.log(12)},200)}>123</el-button>  </> : null}
