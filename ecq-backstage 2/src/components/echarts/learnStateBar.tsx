...
export default defineComponent({
    ...
    setup({title,barData}, ctx) {
        const chartsDOM = ref()
        const store = inject<StoreTypes>('store')!
        let echarts = inject<Echarts>("mechart")!;//引入
        let echart: unknown

        console.log(store.stuLearnState,'console.log(store.stuLearnState)')
        watch([() => store.isPrint,], () => {
            ...
            (option!.series[0].data) = [store.stuBar.stroop +store.stuBar.cognition]
            ...
            series: [
                {
                    ...
                    type: 'bar',
                    barWidth: 20,
                    data:[store.stuBar.stroop +store.stuBar.cognition] ,
        ...
        return () => (
            <>
              <div style='display:none'>{(  option!.series[0].data=[store.stuBar.stroop +store.stuBar.cognition])}</div> 
                <div class='charts3' ref={chartsDOM}></div>
